...
def write_oneof_file(translation_key, task, actions):
    ...
    oneof_file_initial = file("oneof_initial","w")
    oneofcombine_file = file("oneofcombine","w")
    oneofs = task.oneofs
    ors = task.ors
    print("\nors and oneof in initial :")
    print("start")
    # addin
    # for var_no, var_key in enumerate(translation_key):
    #     for value, value_name in enumerate(var_key):
    ...
    for or1 in ors:
     ...
     for i in range(orLen):
      ...
      for var_no, var_key in enumerate(translation_key):
    #    print(var_no, var_key)
       for value, value_name in enumerate(var_key):
        # print(value, value_name,or_valuename_str)
    ...
    if ors != []:
	 print >> oneof_file, "OR\n",
	 print >> oneof_file, "%d\n" % len(ors),
	 print >> oneof_file_initial, "OR\n",
	 print >> oneof_file_initial, "%d\n" % len(ors),
	 for or1 in ors:
	#   or1.dump()
	  orLen = len(or1.parts)  
	#   print(orLen)
	   ...
	   for var_no, var_key in enumerate(translation_key):
	    for value, value_name in enumerate(var_key):
	     if or_valuename_str.find(value_name) != -1:
	      ...
	      print >> oneof_file_initial,"var%d \n" % var_no,
	      tmp = value+1
	      print >> oneof_file,"-%d\n" %tmp,
	      print >> oneof_file_initial,"-%d\n" %tmp,
	    #   print >> oneof_file,", \n",
    ...
    #   print(or_valuename_str)
	   for var_no, var_key in enumerate(translation_key):
        ...
        # print('\n')
	   print >> oneof_file,", \n",
	   print >> oneof_file_initial,", \n",
	  print >> oneof_file, "END_OR\n",
	  print >> oneof_file_initial, "END_OR\n",
	 oneofsize = len(oneofs)
	 print >> oneof_file, "ONEOF\n",
	 print >> oneof_file, "%d\n" % oneofsize,
	 print >> oneof_file_initial, "ONEOF\n",
	 print >> oneof_file_initial, "%d\n" % oneofsize,
	 for oneof in oneofs:
	  ...
	  print >> oneof_file_initial, "END_ONEOF"
	...
	#  print >> oneof_file, "END_ONEOF"
		...
		print >> oneofcombine_file,"%d \n" % oneof_combinable
	    maxlen = 0
	    for oneof in oneofs:
	  	if oneof.get_len() > maxlen:
			maxlen = oneof.get_len()
	    print("maxlen: "+str(maxlen))
	    #oneof_combinable = True 不适用oneof_combinable技术 end
	    axiom_goal = False
			...
			for oneof in oneofs:
				...
				for var_no, var_key in enumerate(translation_key):
					for value, value_name in enumerate(var_key):
							...
							print >> oneof_file, "%d \n" % value,
							flag=1
				oneof_valuename = oneof.negate().print_atom(n),
				oneof_valuename_str = str(oneof_valuename)
				flag = 0
				for var_no, var_key in enumerate(translation_key):
					for value, value_name in enumerate(var_key):
