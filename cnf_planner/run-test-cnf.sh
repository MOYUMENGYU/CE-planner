...
# CNF 批量自动测试脚本
#
# 适配 Conformant_AIJ-DNF/cnf_planner。
#
# 支持两类输入：
#   1. PDDL：domain.pddl + problem.pddl，或 d*.pddl + p*.pddl 成对文件
#      执行方式：cd <cnf_planner> && ./run_cpah.sh <domain> <problem>
#      每次测试后保存 plan-result / result-time / theory_*.al / temp / trash 等文件。
#
#   2. AL：*.al 文件，domain/problem 已经合一
#      执行方式：cd <cnf_planner> && ./cnf <case.al>
#      直接把 CNF 输出保存为该用例的 plan-result。
#
# 默认：
#   timeout = 3600 秒
#   memory  = 16GB
...
常用选项：
  ...
  -t, --test-root DIR     benchmark 根目录，例如 ./CNF-Benchmark-succes 或 ./benchmarks
  --cnf-dir DIR           cnf_planner 目录，默认自动识别
  --cnf-bin FILE          CNF 可执行文件，默认 <cnf-dir>/cnf
  --run-cpah FILE         PDDL 测试脚本，默认 <cnf-dir>/run_cpah.sh
  ...
  -l, --log-root DIR      日志根目录，默认 <root>/cnf_test_logs
  -r, --result-root DIR   结果根目录，默认 <root>/cnf_result_output_batch
  --run-id ID             指定运行批次 ID
  --stop-on-error         遇到失败立即停止
  -h, --help              显示帮助

示例：
  # 脚本放在 cnf_planner 目录下，测试 CNF-Benchmark-succes
  ./run-test-cnf.sh -t ./CNF-Benchmark-succes

  # 只测试一个领域
  ./run-test-cnf.sh -t ./CNF-Benchmark-succes -b 1-dispose-disappear

  # 只测试一个测例
  ./run-test-cnf.sh -t ./CNF-Benchmark-succes -b 1-dispose-disappear -p p2-5

  # 只测试 .al 文件
  ./run-test-cnf.sh -t ./benchmarks --mode al

  # 脚本放在项目根目录
  ./run-test-cnf.sh -t ./benchmarks --cnf-dir ./cnf_planner
USAGE
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --cnf-dir|--cnf-planner-dir) CNF_PLANNER_DIR="$2"; shift 2 ;;
        --cnf-bin) CNF_BIN="$2"; shift 2 ;;
...
# 自动识别 cnf_planner：
#   情况 1：脚本就在 cnf_planner 内，当前目录有 cnf 和 run_cpah.sh
#   情况 2：脚本在项目根目录，子目录 cnf_planner 内有 cnf 和 run_cpah.sh
if [[ -z "$CNF_PLANNER_DIR" ]]; then
    if [[ -x "$ROOT_DIR/cnf" && -f "$ROOT_DIR/run_cpah.sh" ]]; then
        CNF_PLANNER_DIR="$ROOT_DIR"
    else
        CNF_PLANNER_DIR="$ROOT_DIR/cnf_planner"
    fi
fi
CNF_PLANNER_DIR="$(cd "$CNF_PLANNER_DIR" 2>/dev/null && pwd)"
if [[ -z "$CNF_PLANNER_DIR" || ! -d "$CNF_PLANNER_DIR" ]]; then
    echo "[错误] 找不到 cnf_planner 目录，请用 --cnf-dir 指定" >&2
    exit 1
fi

[[ -z "$CNF_BIN" ]] && CNF_BIN="$CNF_PLANNER_DIR/cnf"
[[ -z "$RUN_CPAH" ]] && RUN_CPAH="$CNF_PLANNER_DIR/run_cpah.sh"

if [[ ! -x "$CNF_BIN" ]]; then
    echo "[错误] CNF 可执行文件不存在或不可执行：$CNF_BIN" >&2
    echo "[提示] 执行 chmod +x $CNF_BIN" >&2
    exit 1
fi

if [[ "$MODE" != "al" && ! -x "$RUN_CPAH" ]]; then
    echo "[错误] run_cpah.sh 不存在或不可执行：$RUN_CPAH" >&2
    echo "[提示] 执行 chmod +x $RUN_CPAH $CNF_PLANNER_DIR/sic_script.sh" >&2
    exit 1
fi

# 默认测试目录兼容常见命名
if [[ -z "$TEST_ROOT" ]]; then
    if [[ -d "$ROOT_DIR/CNF-Benchmark-succes" ]]; then
        TEST_ROOT="$ROOT_DIR/CNF-Benchmark-succes"
    elif [[ -d "$CNF_PLANNER_DIR/CNF-Benchmark-succes" ]]; then
        TEST_ROOT="$CNF_PLANNER_DIR/CNF-Benchmark-succes"
...
[[ -z "$LOG_ROOT_BASE" ]] && LOG_ROOT_BASE="$ROOT_DIR/cnf_test_logs"
[[ -z "$RESULT_ROOT_BASE" ]] && RESULT_ROOT_BASE="$ROOT_DIR/cnf_result_output_batch"
...
save_intermediate() {
    ...
    for f in plan-result result-time temp trash dp.pddl pddl2pl.pl new.pl; do
        if [[ -f "$CNF_PLANNER_DIR/$f" ]]; then
            cp -f "$CNF_PLANNER_DIR/$f" "$result_dir/" 2>/dev/null || true
            [[ "$f" == "plan-result" ]] && found_plan_result=0
        fi
    done

    shopt -s nullglob
    for f in "$CNF_PLANNER_DIR"/theory_*.al; do
...
record_result() {
    ...
    elif [[ "$type" == "PDDL" ]] && grep -qiE "syntax error|ERROR:|sicstus: command not found|cannot execute|No such file or directory|theory_0\.al" "$log_file"; then
...
run_pddl_case() {
...

    {
        ...
        echo "[cnf_dir] $CNF_PLANNER_DIR"
        echo "[command] cd $CNF_PLANNER_DIR && ./run_cpah.sh $domain_file $problem_file"
...
run_al_case() {
...

    {
        ...
        echo "[cnf_dir] $CNF_PLANNER_DIR"
        echo "[command] cd $CNF_PLANNER_DIR && ./cnf $al_file"
        echo "[timeout] ${TIMEOUT_SEC}s"
        echo "[mem] ${MEM_GB}GB"
        echo
    } > "$log_file"
...
log_summary "[开始] cnf_dir=$CNF_PLANNER_DIR"
log_summary "[开始] cnf_bin=$CNF_BIN"
...
echo "[开始] cnf_dir=$CNF_PLANNER_DIR"
