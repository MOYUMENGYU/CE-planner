...
#   - 控制台输出完整保存到每个用例独立日志
#   - 不使用宽泛的 "ERROR:" / "failed" 判断失败，避免误判
#   - 以完整 linear 计划为成功依据；允许前面出现内部尝试的 No plan
...
# 检查 CPA(H) 的 linear 计划记录是否完整。
# 支持：
#   linear 3 0 1 2
#   linear 0
# 也兼容动作编号因输出过长而换到后续“纯数字行”的情况。
# 只有 linear 声明的动作数与实际动作编号数一致，才认定为有效计划。
has_valid_linear_plan() {
    local file="$1"

    [[ -s "$file" ]] || return 1

    awk '
    function finish_linear() {
        if (in_linear && numeric_ok && actual == declared) {
            found = 1
        }
        in_linear = 0
        declared = 0
    ...
    in_linear && /^[[:space:]]*[0-9]+([[:space:]]+[0-9]+)*[[:space:]]*$/ {
        actual += NF
        next
    }
...
classify_result() {
    ...
    # GNU timeout 在正常超时时返回 124。137 也可能来自 SIGKILL/OOM，
    # 因此不再无条件把 137 当成普通超时。
    if [[ "$rc" -eq 124 ]]; then
        echo "TIMEOUT"
        return 0
    ...
    # 关键：必须先检查最终有效计划，再检查中间的“No plan”消息。
    # CPA(H) 可能先对若干内部 theory/尝试输出“No plan was found.”，
    # 随后仍然输出最终的 linear 计划。
    if has_valid_linear_plan "$plan_file"; then
        if [[ "$rc" -eq 0 ]]; then
            echo "FOUND_PLAN"
        else
            echo "PLAN_OUTPUT_WITH_EXIT_CODE_${rc}"
        fi
        return 0
    fi

    # 兼容某些 CPA(H) 版本可能使用的显式成功文字，但仍要求退出码为 0。
    if grep -Eiq "Found[[:space:]]+(a[[:space:]]+)?plan|plan[[:space:]]+found" "$plan_file"; then
        if [[ "$rc" -eq 0 ]]; then
            echo "FOUND_PLAN"
        else
            echo "PLAN_OUTPUT_WITH_EXIT_CODE_${rc}"
    ...
    # 只有在没有任何有效最终计划时，No plan 才表示真正失败。
    # 不再使用宽泛的 failed，以免动作名、统计或内部提示造成误判。
    if grep -Eiq "No plan was found|No[[:space:]]+plan|No[[:space:]]+solution|unsolvable|goal.*not.*reachable" "$plan_file"; then
        echo "NO_PLAN_OR_FAILED"
        return 0
    fi

    if [[ "$rc" -eq 137 ]]; then
        echo "KILLED_OR_MEMORY_LIMIT"
        return 0
    fi

    # 退出码 0 但没有可识别计划，不能直接算成功，否则空洞/异常输出会被误报。
    if [[ "$rc" -eq 0 ]]; then
