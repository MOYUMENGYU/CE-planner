...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Compatibility patch for newer SICStus versions.
%% The original CPA(H) package calls remove_duplicates/2, but its definition is
%% commented out in mult5zsic.pl. This predicate removes duplicate terms while
%% preserving the first occurrence order.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
remove_duplicates(List, Result) :-
    remove_duplicates_acc(List, [], Result).

remove_duplicates_acc([], _Seen, []).
remove_duplicates_acc([Head|Tail], Seen, Result) :-
    member_eq(Head, Seen), !,
    remove_duplicates_acc(Tail, Seen, Result).
remove_duplicates_acc([Head|Tail], Seen, [Head|Result]) :-
    remove_duplicates_acc(Tail, [Head|Seen], Result).

member_eq(Item, [Head|_]) :-
    Item == Head, !.
member_eq(Item, [_|Tail]) :-
    member_eq(Item, Tail).
