...
#
# 适用目录：
#   Conformant_AIJ-DNF/pip_planner/
#
# 支持两类测试用例：
#   1) PDDL：
...
#      - d*.pddl + p*.pddl
#      - d*.pddl + p*.pddl~
#      执行：cd pip_planner && ./run_cpah.sh <domain> <problem>
#
#   2) AL：
#      - *.al
#      执行：cd pip_planner && ./pip <case.al>
#
# 关键保证：
#   - 所有 domain/problem/al 路径在执行前转成绝对路径，避免 cd pip_planner 后相对路径失效
#   - 每个用例执行前清理 plan-result、theory_*.al、temp 等中间文件
#   - 每个用例执行后立即保存 plan-result，避免下一用例覆盖
#   - 控制台输出完整保存到每个用例独立日志
#   - 不再用宽泛的 "ERROR:" 判断转换失败，避免误判
#   - 支持 timeout、内存限制、benchmark/problem 过滤、Ctrl+C 优雅退出
#
# 示例：
#   ./run-test-pip.sh -t ./PIP-Benchmark-succes
#   ./run-test-pip.sh -t ./PIP-Benchmark-succes -b blocks
#   ./run-test-pip.sh -t ./PIP-Benchmark-succes -b blocks -p b2
#   ./run-test-pip.sh -t ./benchmarks --mode al
#   ./run-test-pip.sh -t ./PIP-Benchmark-succes --timeout 3600 --mem-gb 16
...
选项：
  ...
  --pip-dir DIR            pip_planner 目录，默认：
                           若脚本当前目录含 ./pip 和 ./run_cpah.sh，则为脚本所在目录；
                           否则为 <root>/pip_planner
  --pip-bin FILE           PIP 可执行文件，默认：<pip_dir>/pip
  -t, --test-root DIR      测试根目录，默认：<root>/Benchmark
  -l, --log-root DIR       日志根目录，默认：<pip_dir>/pip_test_logs
  -r, --result-root DIR    结果根目录，默认：<pip_dir>/pip_result_output_batch
...
输出：
  ...
  PIP结果文件：<result_root>/<run_id>/<benchmark>/<case>/plan-result
...
if [[ -z "$PIP_DIR" ]]; then
    if [[ -x "$ROOT_DIR/pip" && -f "$ROOT_DIR/run_cpah.sh" ]]; then
        PIP_DIR="$ROOT_DIR"
    else
        PIP_DIR="$ROOT_DIR/pip_planner"
    fi
fi

PIP_DIR="$(abs_path_existing_dir "$PIP_DIR")"
if [[ -z "$PIP_DIR" || ! -d "$PIP_DIR" ]]; then
    echo "[错误] 未找到 pip_planner 目录。请用 --pip-dir 指定。" >&2
    exit 1
fi

[[ -z "$PIP_BIN" ]] && PIP_BIN="$PIP_DIR/pip"
if [[ -f "$PIP_BIN" ]]; then
    PIP_BIN="$(abs_path_existing_file "$PIP_BIN")"
fi

[[ -z "$TEST_ROOT" ]] && TEST_ROOT="$ROOT_DIR/Benchmark"
[[ -z "$LOG_ROOT_BASE" ]] && LOG_ROOT_BASE="$PIP_DIR/pip_test_logs"
[[ -z "$RESULT_ROOT_BASE" ]] && RESULT_ROOT_BASE="$PIP_DIR/pip_result_output_batch"

if [[ ! -x "$PIP_BIN" ]]; then
    echo "[错误] 未找到可执行的 pip：$PIP_BIN" >&2
    echo "[提示] 请确认 pip 已存在并 chmod +x，或用 --pip-bin 指定路径。" >&2
    exit 1
fi

if [[ ! -x "$PIP_DIR/run_cpah.sh" ]]; then
    echo "[警告] 未找到可执行的 run_cpah.sh：$PIP_DIR/run_cpah.sh"
...
save_outputs() {
    ...
    for f in plan-result result-time temp trash dp.pddl pddl2pl.pl new.pl; do
        if [[ -f "$PIP_DIR/$f" ]]; then
            cp -f "$PIP_DIR/$f" "$case_result_dir/$f"
            echo "[保存] $f -> $case_result_dir/$f" >> "$case_log_file"
        fi
    done

    shopt -s nullglob
    for f in "$PIP_DIR"/theory_*.al; do
...
detect_fatal_pddl_error() {
    ...
    grep -Eiq \
        "syntax error at symbol|cpa\.pddl2pl.*(No such file|not found|cannot execute|Exec format error|Permission denied)|sicstus: command not found|SICStus.*license|run_cpah\.sh.*(No such file|Permission denied)|sic_script\.sh.*(No such file|Permission denied)|pddl2pl\.pl.*No such file|theory_0\.al.*No such file|cannot execute binary file|Segmentation fault|core dumped" \
        "$log_file"
}
...
classify_result() {
    ...
    if grep -Eiq "Found a plan|plan of length" "$plan_file"; then
        echo "FOUND_PLAN"
        return 0
    fi

    if grep -Eiq "No plan|No solution|unsolvable|goal.*not.*reachable|failed" "$plan_file"; then
...
record_case_result() {
    ...
    if is_success_status "$status"; then
        ...
        echo "[进度] solver=PIP benchmark=$rel_bench_dir case=$case_name type=$type status=$status elapsed=${elapsed}s success=$SUCCESS_CASES failed=$FAILED_CASES total=$TOTAL_CASES/$DISCOVERED_CASES"
    else
...
run_pddl_case() {
    ...
    local problem_base case_log_dir case_result_dir case_log_file
    ...
    domain_file="$(abs_path_existing_file "$domain_file")"
    problem_file="$(abs_path_existing_file "$problem_file")"

    if [[ -z "$domain_file" || -z "$problem_file" ]]; then
    ...
    start_ts=$(date +%s)
    {
        ...
        echo "[command] cd $PIP_DIR && ./run_cpah.sh $domain_file $problem_file"
        echo
    } > "$case_log_file"
    ...
    (
        ...
        exec "$TIMEOUT_CMD" --signal=TERM --kill-after=10s "$TIMEOUT_SEC" \
            ./run_cpah.sh "$domain_file" "$problem_file"
    ) >> "$case_log_file" 2>&1 &
    CURRENT_CHILD_PID=$!
...
run_al_case() {
    ...
    local al_base case_log_dir case_result_dir case_log_file
    local start_ts end_ts elapsed rc result_saved status
    ...
    al_file="$(abs_path_existing_file "$al_file")"
    if [[ -z "$al_file" ]]; then
    ...
    start_ts=$(date +%s)
    {
        ...
        echo "[mem_limit_gb] $MEM_LIMIT_GB"
        echo "[mem_limit_kb_ulimit_v] $MEM_LIMIT_KB"
        echo "[command] cd $PIP_DIR && $PIP_BIN $al_file > plan-result"
        echo
    } > "$case_log_file"
...
log_summary "[开始] PIP_DIR=$PIP_DIR"
log_summary "[开始] PIP_BIN=$PIP_BIN"
...
echo "[开始] solver=PIP run_id=$RUN_ID mode=$MODE timeout=${TIMEOUT_SEC}s mem=${MEM_LIMIT_GB}GB"
echo "[开始] test_root=$TEST_ROOT"
echo "[开始] pip_dir=$PIP_DIR"
